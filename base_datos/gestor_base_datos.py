...
def objeto_lineas_trx(xls:pd.ExcelFile):
    ...
    for fila in tabla_lineas_trx.to_dict("records"):
        ...
        # Extraer variables con valor por defecto de 0 si están vacías o son None
        r = float(fila.get("R") or 0)
        x = float(fila.get("X") or 0)
        b_shunt = float(fila.get("B") or 0)
        g = float(fila.get("G") or 0)
        b_serie = float(fila.get("B_SERIE") or 0)
