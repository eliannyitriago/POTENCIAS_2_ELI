...
class FlujoPotencia:
    ...
    def gauss_seidel(self, max_tolerancia=1e-10, max_ite=10000):
        ...
        #CONDICIONES INICIALES DEL SISTEMA PARA LAS BARRAS
                if (bus.tipo=='slack'):
                    ...
                    if bus.v_ang is not None:
                        #Ya viene un angulo dado (punto de operacion conocido
                        #de antemano, ej. un caso de estudio de un libro) ->
                        #se deja FIJO y no se itera su angulo
                        bus.angulo_fijo = True
                        bus.v_fasor = cmath.rect(bus.v_mag, math.radians(bus.v_ang))
                    else:
                        #Comportamiento de siempre: angulo desconocido,
                        #arranca en 0 y Gauss-Seidel lo calcula
                        bus.angulo_fijo = False
            ...
            while ((error >= max_tolerancia) and (iteraciones <= max_ite)):
                ...
                #APLICACIÓN DE LA FORMULA ITERATIVA
                    if (bus.tipo=='pv'):
                        ...
                        if getattr(bus, 'angulo_fijo', False):
                            #El angulo/voltaje ya viene dado y se deja fijo,
                            #pero Q_neta SI se recalcula cada vuelta (arriba)
                            errores_barras[bus.id_barra] = 0
                            continue
    ...
    def actualizar_potencias(self):
        for barra in self.sistema.barras.values():
            ...
            # 1. Sumar correctamente todas las cargas físicamente conectadas a la barra
            P_dem_total = 0.0
            Q_dem_total = 0.0
            for carga in self.sistema.cargas.values():
                if carga.id_barra == barra.id_barra:
                    P_dem_total += carga.P_mag
                    Q_dem_total += carga.Q_mag
            
            barra.P_dem = P_dem_total
            barra.Q_dem = Q_dem_total
            
            # 2. La física manda: Generación Real = Neta (exportada) + Demandada (local)
            barra.P_gen = barra.P_neta + barra.P_dem
            barra.Q_gen = barra.Q_neta + barra.Q_dem

        # 3. Asignar la potencia base calculada a la entrada mecánica de los generadores
